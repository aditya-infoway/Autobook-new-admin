...
const STATIC_MODELS = [
  { id: 1, model: "ACCESS 125" },
  { id: 2, model: "BURGMAN STREET" },
  { id: 3, model: "GIXXER SF" },
  { id: 4, model: "ACCESS 125 DISC" },
];

// ─── MAIN COMPONENT ─────────────────────────────────────────────────────────

export default function AccessoriesStockReport() {
  ...
  const handleView = (id: number) => {
    navigate(`/accessoriesmaster/stockreport/${id}`);
  };

  // Filter and Pagination Logic
  const filteredData = STATIC_MODELS.filter((item) => {
    const searchLower = search.toLowerCase();
    return item.model.toLowerCase().includes(searchLower);
  });
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="text-xl font-semibold text-gray-900 md:text-2xl dark:text-white">
           Model Wise - Accessories Stock Report
          </h1>
