...
const STATIC_CUSTOMER_TRACKING = [
  {
    id: 1,
...
export default function CustomerTracking() {
 ...
 const handleView = (id: number) => {
    ...
    navigate(`/stock-report/analysis/trackingdetails/${id}`);
  };

  // Filter and Pagination Logic
  const filteredData = STATIC_CUSTOMER_TRACKING.filter((item) => {
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="dark:text-dark-300 mt-1 text-sm text-gray-500">
            Track customer enquiry and quotation status
          </p>
        </div>
  ...
  >
    <ArrowPathIcon className="h-6 w-6 text-gray-600" />
      ...
      <div className="relative w-full max-w-md">
        ...
        <input
          type="text"
          placeholder="Search by customer, number, or quotation..."
          value={search}
          onChange={(e) => {
            ...
            <TBody className="dark:divide-dark-700 divide-y divide-gray-200">
              {currentItems.map((item, index) => (
                <Tr key={item.id} className="dark:hover:bg-dark-700/40 transition-colors hover:bg-gray-50/30 align-middle">
                  ...
                  <Td className="py-3 text-[12px] text-gray-700 dark:text-gray-300">{item.quotationNo}</Td>
