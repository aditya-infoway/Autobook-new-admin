...
// ─── STATIC MOCK DATA (Your requested fields) ─────────────────────────────

const STATIC_TODAY_FOLLOWUPS = [
  {
    id: 1,
    leadId: "L-001",
  ...
  },
  {
    ...
    leadStatus: "Active",
    createdBy: "Rakesh",
    callResponse: "Call Back",
    enquiryStatus: "New Enquiry",
    time: "02:15 PM",
    fCount: 1,
    ageLead: "5 Days",
    lastUpdate: "29-07-2026",
    status: "In Progress",
    bucket: "Attend", // For Grid color
  },
  {
    id: 3,
    leadId: "L-003",
    ...
    createdBy: "Admin",
    callResponse: "Busy",
    enquiryStatus: "Test Drive Done",
    time: "11:45 AM",
    fCount: 4,
    ageLead: "30 Days",
    lastUpdate: "28-07-2026",
    status: "Closed",
    bucket: "Delay", // For Grid color
  },
  {
    id: 4,
    leadId: "L-004",
    ...
    status: "Pending",
    bucket: "Upcoming", // For Grid color
  },
];

// ─── STATIC GRID DATA (For Kanban Board) ──────────────────────────────────

const STATIC_GRID_BOARD = {
  Pending: [STATIC_TODAY_FOLLOWUPS[0], STATIC_TODAY_FOLLOWUPS[3]],
  Attend: [STATIC_TODAY_FOLLOWUPS[1]],
  Delay: [STATIC_TODAY_FOLLOWUPS[2]],
  Upcoming: [],
};
      ...
      <div className="space-y-2.5">
        ...
        <div className="dark:bg-dark-800 flex items-center gap-2 rounded-full bg-gray-50 py-2.5 pr-4 pl-4">
          ...
          <span className="text-xs whitespace-nowrap text-gray-400 dark:text-gray-500">
            {lead.enquiryStatus || "No remarks yet"}
          </span>
        </div>
      </div>
...
const TodayFollowUps: React.FC = () => {
  ...
  const [view, setView] = useState<"table" | "grid">("table"); // Default to table
  const [searchTerm, setSearchTerm] = useState("");
  const [rowsPerPage, setRowsPerPage] = useState(15);
  const [currentPage, setCurrentPage] = useState(1);
  const [collapsed, setCollapsed] = useState(false);

  // ─── Filter & Pagination Logic (Static) ──────────────────────────────────

  const searchLower = searchTerm.toLowerCase();
  const tableData = STATIC_TODAY_FOLLOWUPS.filter((lead) => {
  ...
  const handleViewClick = (id: number) => {
    navigate(`/followup/follow-up/${id}`);
  };

  const handleHistoryClick = (id: number) => {
    navigate(`/followup/followuphistory/${id}`);
  };
    ...
    <div className="dark:bg-dark-800 min-h-screen bg-gray-50 p-4 md:p-6">
      ...
      <div className="mb-6 flex flex-col items-start justify-between md:flex-row md:items-center">
        ...
        <div className="mt-4 flex items-center gap-2 md:mt-0">
          <button className="dark:border-dark-600 dark:bg-dark-700 dark:hover:bg-dark-600 flex h-9 w-9 items-center justify-center rounded-md border border-gray-200 bg-white text-gray-600 transition-all hover:bg-gray-50 dark:text-gray-300">
          ...
          <button
            ...
            className="dark:border-dark-600 dark:bg-dark-700 dark:hover:bg-dark-600 flex h-9 w-9 items-center justify-center rounded-md border border-gray-200 bg-white text-gray-600 transition-all hover:bg-gray-50 dark:text-gray-300"
          >
            <ChevronUp
              className={`h-6 w-6 text-gray-500 transition-transform ${collapsed ? "rotate-180" : ""}`}
            />
          </button>
        </div>
      </div>
          ...
          <div className="dark:bg-dark-600 min-h-100 rounded-xl bg-gray-100 p-4">
            ...
            <div className="max-h-[500px] min-h-0 flex-1 space-y-3 overflow-y-auto pr-1">
              {STATIC_GRID_BOARD.Pending.length === 0 ? (
                ...
                STATIC_GRID_BOARD.Pending.map((lead: any) => (
                  <FollowUpCard
                    key={lead.id}
                    lead={lead}
          ...
          <div className="dark:bg-dark-600 min-h-[400px] rounded-xl bg-gray-100 p-4">
            <div className="dark:bg-dark-700 mb-3 flex items-center justify-between rounded-lg bg-white px-4 py-2 shadow-sm">
              ...
              <span className="rounded-full bg-red-100 px-2.5 py-0.5 text-xs font-semibold text-red-600 dark:bg-red-900/30 dark:text-red-400">
                {STATIC_GRID_BOARD.Delay.length}
              </span>
            </div>
            <div className="max-h-[500px] min-h-0 flex-1 space-y-3 overflow-y-auto pr-1">
              {STATIC_GRID_BOARD.Delay.length === 0 ? (
                <div className="flex h-[300px] items-center justify-center">
                  <span className="text-sm text-gray-400 dark:text-gray-500">
                ...
                STATIC_GRID_BOARD.Delay.map((lead: any) => (
                  <FollowUpCard
                    key={lead.id}
                    lead={lead}
                    onView={handleViewClick}
          ...
          <div className="dark:bg-dark-600 min-h-[400px] rounded-xl bg-gray-100 p-4">
            <div className="dark:bg-dark-700 mb-3 flex items-center justify-between rounded-lg bg-white px-4 py-2 shadow-sm">
              ...
              <span className="rounded-full bg-green-100 px-2.5 py-0.5 text-xs font-semibold text-green-600 dark:bg-green-900/30 dark:text-green-400">
                {STATIC_GRID_BOARD.Attend.length}
              </span>
            </div>
            <div className="max-h-[500px] min-h-0 flex-1 space-y-3 overflow-y-auto pr-1">
              {STATIC_GRID_BOARD.Attend.length === 0 ? (
                <div className="flex h-[300px] items-center justify-center">
                  <span className="text-sm text-gray-400 dark:text-gray-500">
                ...
                STATIC_GRID_BOARD.Attend.map((lead: any) => (
                  <FollowUpCard
                    key={lead.id}
                    lead={lead}
                    onView={handleViewClick}
          ...
          <div className="dark:bg-dark-600 min-h-[400px] rounded-xl bg-gray-100 p-4">
            <div className="dark:bg-dark-700 mb-3 flex items-center justify-between rounded-lg bg-white px-4 py-2 shadow-sm">
              ...
              <span className="rounded-full bg-yellow-100 px-2.5 py-0.5 text-xs font-semibold text-yellow-600 dark:bg-yellow-900/30 dark:text-yellow-400">
                {STATIC_GRID_BOARD.Upcoming.length}
              </span>
            </div>
            <div className="max-h-[500px] min-h-0 flex-1 space-y-3 overflow-y-auto pr-1">
              {STATIC_GRID_BOARD.Upcoming.length === 0 ? (
                <div className="flex h-[300px] items-center justify-center">
                  <span className="text-sm text-gray-400 dark:text-gray-500">
                ...
                STATIC_GRID_BOARD.Upcoming.map((lead: any) => (
                  <FollowUpCard
                    key={lead.id}
                    lead={lead}
                    onView={handleViewClick}
