...
export default function VehicleIncharge() {
  ...
  const handleAction = (item: any) => {
    console.log("Action check clicked for:", item);
  };

  // Filter and Pagination Logic
  const filteredData = STATIC_VEHICLE_INCHARGE.filter((item) => {
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="dark:text-dark-300 mt-1 text-sm text-gray-500">
            Manage vehicle assignment and tracking
          </p>
        </div>
  ...
  >
    <ArrowPathIcon className="h-6 w-6 text-gray-600" />
      ...
      <div className="relative w-full max-w-md">
        ...
        <input
          type="text"
          placeholder="Search by name, mobile, or Q.No..."
          value={search}
          onChange={(e) => {
