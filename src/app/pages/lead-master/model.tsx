/* 
  FULLY STATIC VERSION - NO API CALLS
  All dropdowns (Professions, Enquiry Types, Sources, Statuses, Accounts, Finances, Models, Variants, Colors, Employees) 
  are now hardcoded static arrays.
*/
...
// ─── STATIC DATA (Replaces all API calls) ─────────────────────────────────
const STATIC_MODELS = [
...
const STATIC_SHOWROOM_VARIANTS = [
  { id: 1, modelId: 1, variantName: "DISC RC ABS" },
  { id: 2, modelId: 1, variantName: "DISC" },
  { id: 3, modelId: 2, variantName: "STANDARD" },
  { id: 4, modelId: 3, variantName: "SPORT" },
];

const STATIC_COLORS = [
  { id: 1, colourName: "Pearl Precious White", showroomVariantId: 1 },
  { id: 2, colourName: "Metallic Matte Black", showroomVariantId: 1 },
  { id: 3, colourName: "Pearl Grace White", showroomVariantId: 2 },
  { id: 4, colourName: "Metallic Blue", showroomVariantId: 3 },
];

const STATIC_CUSTOMERS = [
  { id: 1, name: "Jayant Meghnath Dhakul", mobile: "9423025378" },
  { id: 2, name: "Renuka Sudhakar Lad", mobile: "8888811111" },
  { id: 3, name: "Sachin Tendulkar", mobile: "9999988888" },
];

const STATIC_EMPLOYEES = [
  { id: 1, name: "Tanuja Jadhav", role: "Executive" },
  { id: 2, name: "Rakesh Narkhede", role: "Executive" },
  { id: 3, name: "Sachin Patil", role: "Manager" },
];

const STATIC_PROFESSIONS = [
  { id: 1, label: "Engineer", value: "1" },
  { id: 2, label: "Doctor", value: "2" },
  { id: 3, label: "Teacher", value: "3" },
  { id: 4, label: "Businessman", value: "4" },
  { id: 5, label: "Salaried", value: "5" },
];

const STATIC_ENQUIRY_TYPES = [
  { id: 1, label: "New Lead", value: "1" },
  { id: 2, label: "Follow Up", value: "2" },
  { id: 3, label: "Walk In", value: "3" },
];

const STATIC_ENQUIRY_SOURCES = [
  { id: 1, label: "Walk In", value: "1" },
  { id: 2, label: "Referral", value: "2" },
  { id: 3, label: "Online", value: "3" },
  { id: 4, label: "Call Center", value: "4" },
];

const STATIC_ENQUIRY_STATUSES = [
  { id: 1, label: "Pending", value: "1" },
  { id: 2, label: "Completed", value: "2" },
  { id: 3, label: "Follow-up", value: "3" },
  { id: 4, label: "Rejected", value: "4" },
];

const STATIC_ACCOUNTS = [
  { id: 1, name: "Cash Account", group: "Cash-in-Hand" },
  { id: 2, name: "HDFC Bank", group: "Bank Accounts" },
  { id: 3, name: "ICICI Bank", group: "Bank Accounts" },
  { id: 4, name: "Petty Cash", group: "Cash-in-Hand" },
];

const STATIC_FINANCES = [
  { id: 1, name: "XYZ Finance Company", employeeName: "Mr. Sharma" },
  { id: 2, name: "ABC Credit Limited", employeeName: "Ms. Desai" },
];
...
function CreateAccountModal({
  ...
  const customSelectStyles = {
    ...
    input: (provided: any) => ({ ...provided, color: "var(--color-dark-100)" }),
  ...
  const inputClass = (field: string, hasError?: boolean) =>
    `border rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-dark-800 dark:border-dark-500 dark:text-dark-200 ${hasError ? "border-orange-500 dark:border-orange-500" : "border-gray-300"}`;
...
function LeadInfoStep({
  ...
  const inputClass = (field: string) =>
    `border rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-dark-800 dark:border-dark-500 dark:text-dark-200 ${errors[field] ? "border-orange-500 dark:border-orange-500" : "border-gray-300"}`;
      ...
      <label className="flex cursor-pointer items-center gap-1.5">
        <Checkbox
          ...
          onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
            ...
            setFinanceData((prev) => ({ ...prev, wantsFinance: checked }));
          }}
        />
...
function ReviewLeadSummaryStep({
  ...
  const professions = STATIC_PROFESSIONS;
  const enquiryTypes = STATIC_ENQUIRY_TYPES;
  const enquirySources = STATIC_ENQUIRY_SOURCES;
  const enquiryStatuses = STATIC_ENQUIRY_STATUSES;
  const accounts = STATIC_ACCOUNTS;

  const filteredAccounts = accounts.filter((acc: any) => {
    if (form.paymentMode === "CASH") return acc.group === "Cash-in-Hand";
    if (form.paymentMode === "BANK") return acc.group === "Bank Accounts";
    return false;
  });
...
function FinanceDetailsStep({
  ...
  const handleChange = (field: keyof FinanceDetailsType, value: string) => {
    setFinanceDetails((prev) => ({ ...prev, [field]: value }));
  };
  const inputClass =
    "dark:bg-dark-800 dark:border-dark-500 dark:text-dark-200 w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none";
...
export function LeadDetailsModal({
  ...
  // STATIC DROPDOWN STATES
  const [models] = useState(STATIC_MODELS);
  const [showroomVariants] = useState(STATIC_SHOWROOM_VARIANTS);
  const [colors] = useState(STATIC_COLORS);
  const [customers] = useState(STATIC_CUSTOMERS);
  const [executives] = useState(STATIC_EMPLOYEES);
  const [financeOptions] = useState(STATIC_FINANCES);
    ...
    <>
      <Dialog open={isOpen} onClose={onClose} className="relative z-50">
        ...
        <div className="fixed inset-y-0 right-0 flex max-w-full">
          ...
          >
            <div className="dark:bg-dark-600 bg-primary-700 flex items-center justify-between px-4 py-3">
              <DialogTitle className="dark:text-dark-50 font-semibold text-white">
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-start gap-2">
                    <div className="flex-1">
                      ...
                      <Combobox
                        ...
                        columns={[
                          { header: "Customer", field: "name", width: "2fr" },
                          { header: "Mobile", field: "mobile", width: "1.5fr" },
                        ]}
                      />
                      {errors.customer && (
                        <span className="text-xs text-orange-500">
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <Combobox
                      ...
                      columns={[
                        { header: "Name", field: "name", width: "2fr" },
                        { header: "Role", field: "role", width: "1fr" },
                      ]}
                    />
                    {errors.executive && (
                      <span className="text-xs text-orange-500">
            ...
            <div className="dark:border-dark-500 flex justify-between border-t border-gray-200 p-4">
              ...
              {step < 4 && (
                <button
                  type="button"
                  onClick={handleNext}
                  className="bg-primary-600 hover:bg-primary-700 rounded-lg px-6 py-2 text-sm font-semibold text-white"
                >
                  Next
                </button>
              )}
