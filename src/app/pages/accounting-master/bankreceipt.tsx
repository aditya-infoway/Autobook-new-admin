...
const STATIC_BANK_RECEIPTS = [
  {
    id: 1,
    date: "31-07-2026",
    voucherNo: "BR/26-27/2201",
    type: "LBR",
    bankAccount: "SBI Bank",
    oppAccount: "Afreen Raqib Hodekar",
    amount: "2000",
    paymentType: "UPI",
    chequeNo: "",
    chequeDate: "",
  ...
  },
  {
    id: 2,
    date: "31-07-2026",
    voucherNo: "BR/26-27/2200",
    type: "LBR",
    bankAccount: "HDFC Bank",
    oppAccount: "Anupama Hinduja",
    amount: "5000",
    paymentType: "NEFT",
    chequeNo: "",
    chequeDate: "",
  ...
  },
  {
    id: 3,
    date: "31-07-2026",
    voucherNo: "BR/26-27/2199",
    type: "LBR",
    bankAccount: "ICICI Bank",
    oppAccount: "Nilima Kale",
    amount: "3500",
    paymentType: "CHEQUE",
    chequeNo: "CH-101",
    chequeDate: "31-07-2026",
    chequeClearDate: "03-08-2026",
...
const STATIC_OPP_ACCOUNTS = [
  { id: 1, name: "Afreen Raqib Hodekar" },
  { id: 2, name: "Anupama Hinduja" },
  { id: 3, name: "Nilima Kale" },
];
...
const STATIC_JOB_CARDS = [
  { id: 1, name: "JC-2026-001" },
  { id: 2, name: "JC-2026-002" },
  { id: 3, name: "JC-2026-003" },
];
...
function AddBankReceiptDrawer({
  ...
  const [mode, setMode] = useState<"Manual" | "Lead" | "Job Card">("Manual");
  const [paymentType, setPaymentType] = useState<string>("NEFT");
                  ...
                  <div className="flex h-full max-h-screen flex-col bg-white shadow-xl dark:bg-gray-800">
                    ...
                    <div className="flex-1 overflow-y-auto p-4 md:p-6">
                      <div className="space-y-5">
                        {/* Row 1: Radio Buttons & Conditional Input */}
                          ...
                          <div className="flex flex-wrap items-center gap-4">
                            ...
                            <label className="flex cursor-pointer items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
                              <input
                                ...
                                name="mode"
                                value="Job Card"
                                checked={mode === "Job Card"}
                                onChange={() => setMode("Job Card")}
                                className="accent-primary-500"
                              />
                              Job Card
                            </label>
                          ...
                          <div className="w-full sm:w-56">
                            {mode === "Lead" && (
                              <>
                                ...
                                <select className="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm outline-none focus:border-primary-500 focus:ring-1 focus:ring-primary-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white">
                                  <option>Select</option>
                                  {STATIC_LEADS.map((lead) => (
                        ...
                        {/* Row 2: Bank Account | Voucher No | Date */}
                        ...
                        {/* Mode (ALWAYS visible for all modes) */}
                        <div>
                          <label className="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">
                            Mode:
                          </label>
                          <div className="flex flex-wrap gap-4">
                            {["NEFT", "RTGS", "IMPS", "CHEQUE", "UPI"].map((type) => (
                              <label key={type} className="flex cursor-pointer items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
                                <input
                                  type="radio"
                                  name="paymentType"
                                  value={type}
                        ...
                        {/* Cheque Fields (Only if CHEQUE selected) */}
                        {showChequeFields && (
                          <div className="grid grid-cols-1 gap-4 border-t border-gray-200 pt-4 sm:grid-cols-3 dark:border-gray-700">
                            <div>
                              <label className="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">
...
export default function BankReceipt() {
  ...
  const filteredData = STATIC_BANK_RECEIPTS.filter((item) => {
          ...
          <Table className="w-full min-w-[1400px]">
            <THead className="dark:bg-dark-700/60 dark:border-dark-600 border-b border-gray-200 bg-gray-100">
              <Tr>
                ...
                <Th className="text-[11px]">Bank Account</Th>
                <Th className="text-[11px]">Opp. Account</Th>
                <Th className="text-[11px] text-right">Amount</Th>
                <Th className="text-[11px]">Payment Type</Th>
                <Th className="text-[11px]">Cheque No</Th>
                <Th className="text-[11px]">Cheque Date</Th>
                <Th className="text-[11px]">Cheque Clear Date</Th>
            ...
            <TBody className="dark:divide-dark-700 divide-y divide-gray-200">
              {currentItems.map((item, index) => (
                <Tr key={item.id} className="dark:hover:bg-dark-700/40 transition-colors hover:bg-gray-50/30 align-middle">
                  ...
                  <Td className="py-3 text-[12px] text-gray-700 dark:text-gray-300">{item.bankAccount}</Td>
                  <Td className="py-3 text-[12px] text-gray-700 dark:text-gray-300">{item.oppAccount}</Td>
                  <Td className="py-3 text-[12px] font-semibold text-gray-900 dark:text-white text-right">
                    ₹{item.amount}
                  </Td>
                  <Td className="py-3 text-[12px] text-gray-700 dark:text-gray-300">{item.paymentType}</Td>
                  ...
                  <Td className="py-3 text-[12px] text-gray-700 dark:text-gray-300">{item.chequeClearDate || "-"}</Td>
