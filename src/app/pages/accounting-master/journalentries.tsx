...
// ─── ADD JOURNAL ENTRIES DRAWER ────────────────────────────────────────────

function AddJournalEntriesDrawer({
  isOpen,
  onClose,
}: {
  isOpen: boolean;
  ...
  const [rows, setRows] = useState<
    Array<{ party: any; debit: string; credit: string; narration: string }>
  >([
    { party: null, debit: "", credit: "", narration: "" }, // Start with one empty row
  ]);
  ...
  const handleAddRow = () => {
    setRows([...rows, { party: null, debit: "", credit: "", narration: "" }]);
  };
  ...
  const handleRowChange = (
    index: number,
    field: "party" | "debit" | "credit" | "narration",
    value: any,
  ) => {
                            ...
                            <Table className="w-full min-w-[600px]">
                              <THead className="bg-gray-50 dark:bg-gray-800/50">
                                <Tr>
                                  ...
                                  <Th className="w-16 text-center text-[11px]">
                                    ACTION
                                  </Th>
                                </Tr>
                              </THead>
                              <TBody className="divide-y divide-gray-200 dark:divide-gray-700">
                                {rows.map((row, index) => (
                                  <Tr key={index} className="align-middle">
                                    <Td className="px-3 py-2">
                                      <Combobox
                                        data={STATIC_PARTY_ACCOUNTS}
                                        displayField="name"
                                        value={row.party}
                                        onChange={(val: any) =>
                                          handleRowChange(index, "party", val)
                                        }
                                        placeholder="Select"
                                      />
                                    </Td>
                                    ...
                                    <Td className="px-3 py-2 text-center">
                                      {index === rows.length - 1 ? (
                                        <button
                                          ...
                                          className="inline-flex h-8 w-8 items-center justify-center rounded bg-green-600 text-white transition hover:bg-green-700"
                                        >
                                        ...
                                        <button
                                          ...
                                          className="inline-flex h-8 w-8 items-center justify-center rounded bg-red-100 text-red-600 transition hover:bg-red-200"
                                        >
                                ...
                                <Tr className="bg-gray-50 font-semibold dark:bg-gray-800/50">
                                  <Td className="px-3 py-3 text-right font-bold text-gray-700 dark:text-gray-300">
                                    TOTAL
                                  </Td>
...
export default function JournalEntries() {
  ...
  const filteredData = STATIC_JOURNAL_ENTRIES.filter((item) => {
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="dark:text-dark-300 mt-1 text-sm text-gray-500">
            Manage all journal entry records
          </p>
        </div>
          ...
          <Table className="w-full min-w-[1000px]">
            <THead className="dark:bg-dark-700/60 dark:border-dark-600 border-b border-gray-200 bg-gray-100">
              <Tr>
                ...
                <Th className="text-[11px]">Account</Th>
                <Th className="text-right text-[11px]">Debit</Th>
                <Th className="text-right text-[11px]">Credit</Th>
