...
export default function Model() {
  ...
  const handleOpenAddDrawer = () => {
    setEditId(null);
    setFormData({ modelName: "", category: "", status: "ACTIVE" });
    setShowDrawer(true);
  };
  ...
  const handleDelete = (id: number) => {
    alert(`Delete model with ID: ${id}`);
  };

  const handleToggleStatus = (id: number) => {
    alert(`Toggle status for model ID: ${id}`);
  };

  const handleBulkDelete = () => {
    alert(`Delete ${selectedIds.length} selected models`);
  };

  const categoryFilterOptions = [
    { id: "All", name: "All" },
    ...Array.from(new Set(models.map((c) => c.category))).map((n) => ({
      id: n,
      name: n,
    })),
  ];
            ...
            <div className="flex flex-col gap-1">
              ...
              <div className="w-full">
                <Combobox
                  ...
                  onChange={(value: any) => {
                    setSelectedCategoryFilter(value.id);
                    setCurrentPage(1);
                  }}
                    ...
                    <Listbox
                      ...
                      value={
                        categoryOptions.find(
                          (opt) => opt.name === formData.category,
                        ) || null
                      }
                      placeholder="Select Category"
                      onChange={(value) => {
