...
// Static Data
const exchangeBonusData = [
  {
    id: 1,
    model: "iPhone 15 Pro",
    exchangeBonusAmount: 5000,
    companyShare: 3000,
    dealerShare: 2000,
...
export default function ExchangeBonus() {
  ...
  const exchangeBonuses = exchangeBonusData;
  ...
  const filteredData = exchangeBonuses.filter((item) => {
  ...
  const handleOpenEditDrawer = (item: any) => {
    ...
    setFormData({
      model: item.model,
      exchangeBonusAmount: String(item.exchangeBonusAmount),
      companyShare: String(item.companyShare),
      dealerShare: String(item.dealerShare),
  ...
  const handleDelete = (id: number) => {
    alert(`Delete exchange bonus with ID: ${id}`);
  };

  const handleBulkDelete = () => {
    alert(`Delete ${selectedIds.length} selected exchange bonuses`);
  };

  const modelFilterOptions = [
    { id: "All", name: "All" },
    ...Array.from(new Set(exchangeBonuses.map((c) => c.model))).map((n) => ({
      id: n,
      name: n,
    })),
  ];
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="dark:text-dark-300 mt-1 text-sm text-gray-500">
            Manage all exchange bonuses from here
          </p>
        </div>
  ...
  >
    Add Exchange Bonus
      ...
      <div className="relative w-full max-w-md">
        ...
        <input
          type="text"
          placeholder="Search exchange bonus..."
          value={search}
          onChange={(e) => {
                ...
                return (
                  ...
                  >
                    ...
                    <Td className="py-4 text-gray-600 dark:text-gray-400">
                      ₹{item.exchangeBonusAmount.toLocaleString()}
                    </Td>
                    <Td className="py-4 text-gray-600 dark:text-gray-400">
                      ₹{item.companyShare.toLocaleString()}
                    </Td>
                    <Td className="py-4 text-gray-600 dark:text-gray-400">
                      ₹{item.dealerShare.toLocaleString()}
                    </Td>
                ...
                <div className="grow overflow-y-auto p-5">
                  <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                    ...
                    <div>
                      ...
                      <Input
                        type="number"
                        placeholder="Enter exchange bonus amount"
                        value={formData.exchangeBonusAmount}
                        onChange={(e) =>
                          setFormData({
                            ...formData,
                            exchangeBonusAmount: e.target.value,
                          })
                        }
                        className="w-full"
                      />
                    ...
                    <div>
                      <label className="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">
                        Company Share <span className="text-red-500">*</span>
                      </label>
                      <Input
                        type="number"
                    ...
                    <div>
                      <label className="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">
                        Dealer Share <span className="text-red-500">*</span>
                      </label>
                      <Input
                        type="number"
                        placeholder="Enter dealer share"
                        value={formData.dealerShare}
                        onChange={(e) =>
                          setFormData({
                            ...formData,
