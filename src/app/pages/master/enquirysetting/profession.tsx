...
export default function Profession() {
  ...
  const handleOpenAddDrawer = () => {
    setEditId(null);
    setFormData({ profession: "", status: "ACTIVE" });
    setShowDrawer(true);
  };
  ...
  const handleDelete = (id: number) => {
    alert(`Delete profession with ID: ${id}`);
  };

  const handleToggleStatus = (id: number) => {
    alert(`Toggle status for profession ID: ${id}`);
  };

  const handleBulkDelete = () => {
    alert(`Delete ${selectedIds.length} selected professions`);
  };
