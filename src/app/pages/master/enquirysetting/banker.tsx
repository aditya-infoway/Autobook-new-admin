...
export default function Banker() {
  ...
  const filteredData = bankers.filter((item) => {
    const matchesSearch = item.banker
      .toLowerCase()
  ...
  const handleOpenAddDrawer = () => {
    setEditId(null);
    setFormData({ banker: "", status: "ACTIVE" });
    setShowDrawer(true);
  };
  ...
  const handleDelete = (id: number) => {
    alert(`Delete banker with ID: ${id}`);
  };

  const handleToggleStatus = (id: number) => {
    alert(`Toggle status for banker ID: ${id}`);
  };

  const handleBulkDelete = () => {
    alert(`Delete ${selectedIds.length} selected bankers`);
  };
