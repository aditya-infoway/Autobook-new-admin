...
export default function EnquirySource() {
  ...
  const enquirySources = enquirySourceData;
  ...
  const handleOpenAddDrawer = () => {
    setEditId(null);
    setFormData({ source: "", status: "ACTIVE" });
    setShowDrawer(true);
  };
  ...
  const handleDelete = (id: number) => {
    alert(`Delete enquiry source with ID: ${id}`);
  };

  const handleToggleStatus = (id: number) => {
    alert(`Toggle status for enquiry source ID: ${id}`);
  };

  const handleBulkDelete = () => {
    alert(`Delete ${selectedIds.length} selected enquiry sources`);
  };
