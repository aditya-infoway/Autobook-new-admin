...
export default function EnquiryType() {
  ...
  const enquiryTypes = enquiryTypeData;
  ...
  const filteredData = enquiryTypes.filter((item) => {
    const matchesSearch = item.enquiryType
      .toLowerCase()
  ...
  const handleOpenAddDrawer = () => {
    setEditId(null);
    setFormData({ enquiryType: "", status: "ACTIVE" });
    setShowDrawer(true);
  };
  ...
  const handleDelete = (id: number) => {
    alert(`Delete enquiry type with ID: ${id}`);
  };

  const handleToggleStatus = (id: number) => {
    alert(`Toggle status for enquiry type ID: ${id}`);
  };

  const handleBulkDelete = () => {
    alert(`Delete ${selectedIds.length} selected enquiry types`);
  };
  ...
  <Combobox
    ...
    displayField="name"
    value={statusFilterOptions.find(opt => opt.id === selectedStatusFilter) || statusFilterOptions[0]}
    onChange={(value: any) => {
