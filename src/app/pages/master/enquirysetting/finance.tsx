...
const financeData = [
  ...
  },
  {
    id: 2,
    financer: "ICICI Bank",
    salesExecutive: "Ankita Ghavanalkar",
    status: "ACTIVE",
...
const financerOptions = [
  ...
  { id: 7, name: "Punjab National Bank" },
...
const salesExecutiveOptions = [
  { id: 1, name: "Rohit Parab" },
  { id: 2, name: "Ankita Ghavanalkar" },
  { id: 3, name: "Rakesh Naik" },
  { id: 4, name: "tanisha kadam" },
  { id: 5, name: "mamta samant" },
  { id: 6, name: "Prakash Kudtarkar" },
];
...
export default function Finance() {
  ...
  const [selectedFinancerFilter, setSelectedFinancerFilter] =
    useState<string>("All");
  ...
  const filteredData = finances.filter((item) => {
    const matchesSearch =
      item.financer.toLowerCase().includes(search.toLowerCase()) ||
      item.salesExecutive.toLowerCase().includes(search.toLowerCase());
    const matchesFinancer =
      selectedFinancerFilter === "All" ||
      item.financer === selectedFinancerFilter;
    const matchesStatus =
      selectedStatusFilter === "All" || item.status === selectedStatusFilter;
    return matchesSearch && matchesFinancer && matchesStatus;
  });
  ...
  const handleDelete = (id: number) => {
    alert(`Delete finance with ID: ${id}`);
  };

  const handleToggleStatus = (id: number) => {
    alert(`Toggle status for finance ID: ${id}`);
  };

  const handleBulkDelete = () => {
    alert(`Delete ${selectedIds.length} selected finances`);
  };

  const financerFilterOptions = [
    { id: "All", name: "All" },
    ...Array.from(new Set(finances.map((c) => c.financer))).map((n) => ({
      id: n,
      name: n,
    })),
  ];
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div className="flex flex-col gap-1">
              ...
              <div className="w-full">
                <Combobox
                  data={financerFilterOptions}
                  displayField="name"
                  value={
                    financerFilterOptions.find(
                      (opt) => opt.id === selectedFinancerFilter,
                    ) || financerFilterOptions[0]
                  }
                  onChange={(value: any) => {
                    setSelectedFinancerFilter(value.id);
                    setCurrentPage(1);
                  }}
              ...
              >
                ...
                <div className="grow space-y-5 overflow-y-auto p-5">
                  <div>
                    ...
                    <Combobox
                      data={financerOptions}
                      displayField="name"
                      value={financerOptions.find(
                        (opt) => opt.name === formData.financer,
                      )}
                      onChange={(selected: any) => {
                        setFormData({
                          ...formData,
                          financer: selected?.name || "",
                        });
                      }}
                      placeholder="Select Financer"
                      searchFields={["name"]}
