...
export default function SchemeType() {
  ...
  const schemeTypes = schemeTypeData;
  ...
  const filteredData = schemeTypes.filter((item) => {
    const matchesSearch = item.schemeType
      .toLowerCase()
  ...
  const handleOpenAddDrawer = () => {
    setEditId(null);
    setFormData({ schemeType: "", status: "ACTIVE" });
    setShowDrawer(true);
  };
  ...
  const handleDelete = (id: number) => {
    alert(`Delete scheme type with ID: ${id}`);
  };

  const handleToggleStatus = (id: number) => {
    alert(`Toggle status for scheme type ID: ${id}`);
  };

  const handleBulkDelete = () => {
    alert(`Delete ${selectedIds.length} selected scheme types`);
  };
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="dark:text-dark-300 mt-1 text-sm text-gray-500">
            Manage all scheme types from here
          </p>
        </div>
              ...
              >
                <div className="bg-primary-500 flex items-center justify-between px-5 py-4">
                  <h2 className="text-lg font-semibold text-white">
                    {editId !== null ? "Edit Scheme Type" : "Add Scheme Type"}
                  </h2>
                  <Button
