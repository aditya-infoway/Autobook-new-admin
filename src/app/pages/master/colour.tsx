...
export default function Colour() {
  ...
  const handleOpenEditDrawer = (item: any) => {
    ...
    setFormData({
      ...
      colourPicker: item.colourPicker,
      status: item.status,
    });
    setShowDrawer(true);
  };
  ...
  const handleDelete = (id: number) => {
    alert(`Delete colour with ID: ${id}`);
  };

  const handleToggleStatus = (id: number) => {
    alert(`Toggle status for colour ID: ${id}`);
  };

  const handleBulkDelete = () => {
    alert(`Delete ${selectedIds.length} selected colours`);
  };

  const modelFilterOptions = [
    { id: "All", name: "All" },
    ...Array.from(new Set(colours.map((c) => c.model))).map((n) => ({
      id: n,
      name: n,
    })),
  ];
    ...
    <div className="relative min-h-screen space-y-6 p-4 pb-28 text-gray-900 md:p-6 dark:text-gray-100">
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2 sm:flex-nowrap">
          ...
          >
            <FaFileExcel className="text-lg text-green-500" />
          </button>
                    ...
                    <Td className="py-4">
                      <div className="flex items-center gap-2">
                        <div
                          ...
                          style={{ backgroundColor: item.colourPicker }}
                        />
                ...
                <div className="grow space-y-5 overflow-y-auto p-5">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-3">
                      <input
                        type="color"
                        value={formData.colourPicker}
                        onChange={(e) =>
                          setFormData({
                            ...formData,
                            colourPicker: e.target.value,
                          })
                        }
                        className="dark:border-dark-500 h-12 w-12 cursor-pointer rounded-lg border border-gray-300 p-1"
                      />
                      <Input
                        type="text"
                        placeholder="#000000"
